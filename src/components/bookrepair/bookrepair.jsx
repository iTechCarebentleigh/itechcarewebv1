...
const Bookrepair = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const currentMoment = moment.tz('Australia/Sydney');
    
    // Check if the booking time is in the future
    if (!visitDateMoment.isAfter(currentMoment)) {
        toast.custom((t) => (
            <div style={{ fontFamily: "var(--typography-font-family-sansserif)" }} className="w-full md:w-[350px] bg-amber-50 border border-amber-300 px-3 py-4 text-amber-500 rounded-xl shadow-2xl">
                <div className="flex gap-2 items-center">
                    ...
                    <span className='text-md'>Please enter a date in the future.</span>
                </div>
            </div>
        ));
        setQueryLoading(false); // Reset loading state
